 ...
 * @returns {Promise<Object>} - The customers list with pagination info.
 */
export const fetchAllCustomers = async (skip = 0, limit = 100) => {
    try {
        const response = await apiClient.get(`/api/churn/customers?skip=${skip}&limit=${limit}`);
        return response.data;
    } catch (error) {
        handleApiError(error);
        throw error;
