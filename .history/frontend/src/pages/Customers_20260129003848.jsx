...
const Customers = () => {
    ...
    useEffect(() => {
        const loadCustomers = async () => {
            try {
                setLoading(true);
                const response = await fetchAllCustomers(0, 100); // Fetch first 100 customers
    ...
    const filteredCustomers = useMemo(() => {
        if (!searchTerm.trim()) {
            return customers.slice(0, displayLimit); // Show only first 15
        }
        ...
        return filtered.slice(0, displayLimit); // Show only first 15 filtered results
